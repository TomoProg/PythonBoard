...
def index(request):
    """ index.html view """
    template = loader.get_template('PythonBoard/index.html')

    topic_num_query = """
                      SELECT
                          COUNT(*)
                      FROM
                          PythonBoard_topicdetail
                      WHERE
                          PythonBoard_topic.id = PythonBoard_topicdetail.title_id
                      """
...
def about(request):
    """ about.html view """
    template = loader.get_template('PythonBoard/about/about.html')
    context = {
        'signin_username': request.user.username,
    }
    return HttpResponse(template.render(context, request))
...
def contact(request):
    """ contact.html view """
    template = loader.get_template('PythonBoard/contact/contact.html')
    context = {
        'signin_username': request.user.username,
    }
    return HttpResponse(template.render(context, request))
...
@login_required(login_url='PythonBoard:signin')
def create(request):
    """ create.html view """
    template = loader.get_template('PythonBoard/create/create.html')
    f = TopicForm()
    ...
    if request.method == 'POST':
        ...
        if f.is_valid():
            ...
            td.save()
            return redirect('PythonBoard:index')
    context = {
        'signin_username': request.user.username,
        'form': f,
    }
...
def reply(request, topic_id, text_no=0):
    """ reply.html view """
    template = loader.get_template('PythonBoard/reply/reply.html')
    ...
    if request.method == 'POST':
        ...
        if f.is_valid():
            ...
            t.save()
            return redirect('PythonBoard:detail', topic_id)
    else:
        t = Topic.objects.get(pk=topic_id)
        topic_title = t.title
...
def signin(request):
    """ signin.html view """
    template = loader.get_template('PythonBoard/signin/signin.html')
    f = AccountForm()
    error_message = ''
...
def signup(request):
    """ signup.html view """
    template = loader.get_template('PythonBoard/signup/signup.html')
    f = AccountForm()
    error_message = ''
    ...
    if request.method == 'POST':
        ...
        if f.is_valid():
            try:
                ...
                user.save()
                return redirect('PythonBoard:signin')
